...
export async function handleGitStatus(cwd, res) {
  ...
  if (!cwdStr) {
    return sendJson(res, { data: { isGit: false, cwd: '' } });
  }
  ...
  if (!isGit) {
    return sendJson(res, { data: { isGit: false, cwd: cwdStr } });
  }
...
export async function handleGitBranches(cwd, refresh, res) {
  ...
  sendJson(res, { data: { local, remote, current } });
}
...
export async function handleGitCheckout(cwd, req, res) {
  ...
  return withJsonBody(req, async (body) => {
    ...
    if (checkoutResult.code === 0) {
      return sendJson(res, { data: { ok: true, branch } });
    }

    sendJson(res, { data: null, error: checkoutResult.stderr || 'git checkout 失败' });
  });
}
