 ...
 * **但实测不是这样**：2026-08-26 对 kxmall 的 style-system.md（124 行）跑真实调用，
 * 默认模型耗时 54.5s——原定的 60s 预算只剩 5.5s 余量（用掉 90%）。
 * 稍大一点的规范、或赶上限流排队，就会踩线超时。
 *
 ...
 *
 * 教训沿用 check-prompts 的 BATCH_TIMEOUT_MS：超时预算是**模型速度的依赖变量**，
 * 必须按实测校准而不是按「输出很短所以应该很快」拍脑袋——这里输出短但思考不短。
 */
